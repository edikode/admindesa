							...
							<div class="col-md-12">
								...
								<div class="row">
									<div class="col-md-8">
										<div class='form-group @if($errors->has('tmp_dikeluarkan')) has-error @endif'>
											...
											<input type='text' placeholder='Tempat Dikeluarkan' class='form-control limited' id='tmp_dikeluarkan' name='tmp_dikeluarkan' maxlength='100' value='@if(count($errors) > 0){{old('tmp_dikeluarkan')}}@else{{$pd->tmp_dikeluarkan}}@endif' required>
									...
									<div class="col-md-4">
										<div class='form-group @if($errors->has('tgl_dikeluarkan')) has-error @endif'>
											...
											<div class="input-group">
												<input name="tgl_dikeluarkan" type="text" data-date-format="dd-mm-yyyy" data-date-viewmode="years" class="form-control date-picker" value="@if(count($errors) > 0){{old('tgl_dikeluarkan')}}@else{{tgl_id($pd->tgl_dikeluarkan)}}@endif" required>
								...
								<div class='form-group @if($errors->has('status_dikeluarga')) has-error @endif'>
									...
									<select name="status_dikeluarga" id="status_dikeluarga" class="form-control" required>
										...
										<option value="Ayah" 
											@if(count($errors) > 0) 
												@if($errors->first('status_dikeluarga') == "Ayah") selected @endif 
											@else 
												@if($pd->status_dikeluarga == "Ayah") selected @endif
											@endif>Ayah</option>
										<option value="Ibu"@if(count($errors) > 0) 
												@if($errors->first('status_dikeluarga') == "Ibu") selected @endif 
											@else 
												@if($pd->status_dikeluarga == "Ibu") selected @endif
											@endif>Ibu</option>
										<option value="Anak" @if(count($errors) > 0) 
												@if($errors->first('status_dikeluarga') == "Anak") selected @endif 
											@else 
												@if($pd->status_dikeluarga == "Anak") selected @endif
											@endif>Anak</option>
										<option value="Lainnya" 
											@if(count($errors) > 0) 
												@if($errors->first('status_dikeluarga') == "Lainnya") selected @endif 
											@else 
								...
								<div class='form-group @if($errors->has('ayah')) has-error @endif'>
									...
									<input type='text' placeholder='Nama Ayah' class='form-control limited' id='ayah' name='ayah' maxlength='100' value='@if(count($errors) > 0){{old('ayah')}}@else{{$pd->ayah}}@endif'>
								...
								<div class='form-group @if($errors->has('ibu')) has-error @endif'>
									...
									<input type='text' placeholder='Nama Ibu' class='form-control limited' id='ibu' name='ibu' maxlength='100' value='@if(count($errors) > 0){{old('ibu')}}@else{{$pd->ibu}}@endif'>
								...
								<div class='form-group @if($errors->has('tgl_didesa')) has-error @endif'>
									<div class="input-group">
										<input name="tgl_didesa" type="text" data-date-format="dd-mm-yyyy" data-date-viewmode="years" class="form-control date-picker" value="@if(count($errors) > 0){{old('tgl_didesa')}}@else{{tgl_id($pd->tgl_didesa)}}@endif" required>
										<span class="input-group-addon"> <i class="fa fa-calendar"></i> </span>
									</div>
