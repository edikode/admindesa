							...
							@foreach ($agenda as $p)
								<tr>
									...
									@if ($p->jenis == "Surat Masuk")
										<td>{{tgl_id($p->tanggal_surat)}}</td>
										<td>{{$p->nomor}}</td>
										<td>{{tgl_id($p->tanggal)}}</td>
										<td>{{$p->pengirim}}</td>
										<td>{{$p->isi}}</td>
										<td align="center">-</td>
										<td align="center">-</td>
					...
					<div class="col-md-12">
						<div class='form-group @if($errors->has('jenis')) has-error @endif'>
							...
							<select name="jenis" id="jenis" class="form-control" required>
								...
								<option value="Surat Masuk" @if(count($errors) > 0) @if(old('jenis') == "Surat Masuk") selected @endif @endif>Surat Masuk</option>
								<option value="Surat Keluar" @if(count($errors) > 0) @if(old('jenis') == "Surat Keluar") selected @endif @endif>Surat Keluar</option>
							</select>
						...
						<div class='form-group @if($errors->has('tanggal_surat')) has-error @endif'>
							...
							<div class="input-group">
								<input name="tanggal_surat" type="text" data-date-format="dd-mm-yyyy" data-date-viewmode="years" class="form-control date-picker" value="@if(count($errors) > 0){{old('tanggal_surat')}}@endif" placeholder="Tanggal Penerimaan/ Pengiriman Surat" required autocomplete="off">
						...
						<div class='form-group @if($errors->has('nomor')) has-error @endif'>
							...
							<input type='text' placeholder='Nomor' class='form-control limited' id='nomor' name='nomor' maxlength='45' value='@if(count($errors) > 0){{old('nomor')}}@endif'
							required>
						...
						<div class='form-group @if($errors->has('tanggal')) has-error @endif'>
							...
							<div class="input-group">
								<input name="tanggal" type="text" data-date-format="dd-mm-yyyy" data-date-viewmode="years" class="form-control date-picker" value="@if(count($errors) > 0){{old('tanggal')}}@endif" placeholder="Tanggal Surat Masuk/Keluar" required autocomplete="off">
								<span class="input-group-addon"> <i class="fa fa-calendar"></i> </span>
							</div>
						...
						<div class='form-group @if($errors->has('pengirim')) has-error @endif'>
							...
							<input type='text' placeholder='Pengirim/Penerima' class='form-control limited' id='pengirim' name='pengirim' maxlength='45' value='@if(count($errors) > 0){{old('pengirim')}}@endif'>
