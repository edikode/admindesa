							...
							<div class="col-md-12">
								<div class='form-group @if($errors->has('tanggal_pengiriman')) has-error @endif'>
									...
									<div class="input-group">
										<input name="tanggal_pengiriman" type="text" data-date-format="dd-mm-yyyy" data-date-viewmode="years" class="form-control date-picker" value="@if(count($errors) > 0){{old('tanggal_pengiriman')}}@else{{tgl_id($pd->tanggal_pengiriman)}}@endif" placeholder="Tanggal Pengiriman" required autocomplete="off">
										<span class="input-group-addon"> <i class="fa fa-calendar"></i> </span>
									</div>

									@if ($errors->has('tanggal_pengiriman'))
										<span for="tanggal_pengiriman" class="help-block">{{ $errors->first('tanggal_pengiriman') }}</span>
									@endif
								</div>
								<div class='form-group @if($errors->has('tanggal_surat')) has-error @endif'>
									<label class='control-label'>Tanggal Dan Nomor Surat</label>
									<input type='text' placeholder='Tanggal Dan Nomor Surat' class='form-control limited' id='tanggal_surat' name='tanggal_surat' maxlength='45' value='@if(count($errors) > 0){{old('tanggal_surat')}}@else{{$pd->tanggal_surat}}@endif'
									required>
								...
								<div class='form-group @if($errors->has('tujuan')) has-error @endif'>
									...
									<input type='text' placeholder='Ditujukan Kepada' class='form-control limited' id='tujuan' name='tujuan' maxlength='45' value='@if(count($errors) > 0){{old('tujuan')}}@else{{$pd->tujuan}}@endif'
									required>
