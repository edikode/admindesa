...
@section('main')
				...
				<div class="row">
					<div class="col-md-12">
						<div class='form-group @if($errors->has('jenis')) has-error @endif'>
							...
							<input type='text' placeholder='Jenis Barang/Bangunan' class='form-control limited' id='jenis' name='jenis' maxlength='45' value='@if(count($errors) > 0){{old('jenis')}}@endif'
							required>
	...
	<div class="modal fade modal-crud" id="hapus" tabindex="-1" role="dialog" aria-hidden="true">
			...
			<div class="modal-body">
				<div class="row">
					<div class="col-md-12">
						<div class='form-group @if($errors->has('jenis')) has-error @endif'>
							<label class='control-label'>Jenis Barang/Bangunan</label>
							<select name="jenis" id="jenis" class="form-control search-select" required>
								<option value="">-- Pilih --</option>
								@foreach ($data as $d)
									@php if(CekData("inventaris_desa","id",$d->id)){ continue; } @endphp
						...
						<div class='form-group @if($errors->has('tgl_hapus')) has-error @endif'>
							...
							<div class="input-group">
								<input name="tgl_hapus" placeholder="Tanggal Dihapus" type="text" data-date-format="dd-mm-yyyy" data-date-viewmode="years" class="form-control date-picker" value="@if(count($errors) > 0){{old('tgl_hapus')}}@endif" required autocomplete="off">
