						...
						<tbody>
							...
							@foreach ($tanahkasdesa as $p)
								<tr>
									...
									<td>{{$p->lokasi}}</td>
									<td>{{$p->peruntukan}}</td>
									<td>@if($p->mutasi){{$p->mutasi}}@else - @endif</td>
									{{-- <td>{{$p->keterangan}}</td> --}}
									<td>
				...
				<div class="row">
					<div class="col-md-12">
						<div class='form-group @if($errors->has('asal')) has-error @endif'>
							...
							<input type='text' placeholder='Asal Tanah Kas Desa' class='form-control limited' id='asal' name='asal' maxlength='45' value='@if(count($errors) > 0){{old('asal')}}@endif'
							required>
						...
						<div class='form-group @if($errors->has('nomor_sertifikat')) has-error @endif'>
							...
							<input type='text' placeholder='nomor sertifikat, atau buku Letter C, atau Persil' class='form-control limited' id='nomor_sertifikat' name='nomor_sertifikat' maxlength='45' value='@if(count($errors) > 0){{old('nomor_sertifikat')}}@endif'
							required>
						...
						<div class='form-group @if($errors->has('luas')) has-error @endif'>
							...
							<input type='text' placeholder='luas tanah kas Desa dalam meter persegi (M2)' class='form-control limited' id='luas' name='luas' maxlength='10' value='@if(count($errors) > 0){{old('luas')}}@endif'
							required>
						...
						<div class='form-group @if($errors->has('kelas')) has-error @endif'>
							...
							<label class='radio-inline'>
								<input type='radio' name='kelas' class='square-green' value='S1' @if(count($errors) > 0) checked @endif>
								S1
							</label>
							<label class='radio-inline'>
								<input type='radio' name='kelas' class='square-green' value='D1' @if(count($errors) > 0) checked @endif>
								D1
							</label>
							<label class='radio-inline'>
								<input type='radio' name='kelas' class='square-green' value='Lainnya' @if(count($errors) > 0) checked @endif>
								Lainnya
							</label>
						...
						<div class='form-group @if($errors->has('tgl_perolehan')) has-error @endif'>
							...
							<div class="input-group">
								<input name="tgl_perolehan" type="text" data-date-format="dd-mm-yyyy" data-date-viewmode="years" class="form-control date-picker" value="@if(count($errors) > 0){{old('tgl_perolehan')}}@endif" placeholder="Tanggal Kesepakatan" required autocomplete="off">
						...
						<div class='form-group @if($errors->has('papan_nama')) has-error @endif'>
							...
							<br>
							<label class='radio-inline'>
								<input type='radio' name='papan_nama' class='square-green' value='Sudah Ada'  @if(count($errors) > 0) checked @endif>
								Sudah Ada
							</label>
							<label class='radio-inline'>
								<input type='radio' name='papan_nama' class='square-green' value='Belum Ada'  @if(count($errors) > 0) checked @endif>
								Belum Ada
							</label>
						...
						<div class='form-group @if($errors->has('peruntukan')) has-error @endif'>
							...
							<input type='text' placeholder='Peruntukan/pemanfaatan tanah kas Desa' class='form-control limited' id='peruntukan' name='peruntukan' maxlength='45' value='@if(count($errors) > 0){{old('peruntukan')}}@endif'
							required>
						...
						<div class='form-group @if($errors->has('mutasi')) has-error @endif'>
							...
							<input type='text' placeholder='Diisi setiap terjadi mutasi tanah kas Desa' class='form-control limited' id='mutasi' name='mutasi' maxlength='45' value='@if(count($errors) > 0){{old('mutasi')}}@endif'>
