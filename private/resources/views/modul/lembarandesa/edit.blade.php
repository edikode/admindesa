						...
						<div class="row">
							...
							<div class="col-md-12">
								<div class='form-group @if($errors->has('jenis')) has-error @endif'>
									...
									<input type='text' placeholder='Jenis' class='form-control limited' id='jenis' name='jenis' maxlength='45' value='@if(count($errors) > 0){{old('jenis')}}@else{{$ld->jenis}}@endif'
									required>
								...
								<div class='form-group @if($errors->has('nomor_ditetapkan')) has-error @endif'>
									...
									<input type='text' placeholder='Nomor Ditetapkan' class='form-control limited' id='nomor_ditetapkan' name='nomor_ditetapkan' maxlength='45' value='@if(count($errors) > 0){{old('nomor_ditetapkan')}}@else{{$ld->nomor_ditetapkan}}@endif' required>
								...
								<div class='form-group @if($errors->has('tanggal_diundangkan')) has-error @endif'>
									...
									<div class="input-group">
										<input name="tanggal_diundangkan" type="text" data-date-format="dd-mm-yyyy" data-date-viewmode="years" class="form-control date-picker" value="@if(count($errors) > 0){{old('tanggal_diundangkan')}}@else{{tgl_id($ld->tanggal_diundangkan)}}@endif" placeholder="Tanggal Diundangkan" required autocomplete="off">
										<span class="input-group-addon"> <i class="fa fa-calendar"></i> </span>
									</div>

									@if ($errors->has('tanggal_diundangkan'))
										<span for="tanggal_diundangkan" class="help-block">{{ $errors->first('tanggal_diundangkan') }}</span>
									@endif
								</div>
								<div class='form-group @if($errors->has('nomor_diundangkan')) has-error @endif'>
									<label class='control-label'>Nomor Diundangkan</label>
									<input type='text' placeholder='Nomor Diundangkan' class='form-control limited' id='nomor_diundangkan' name='nomor_diundangkan' maxlength='45' value='@if(count($errors) > 0){{old('nomor_diundangkan')}}@else{{$ld->nomor_diundangkan}}@endif'
									required>
